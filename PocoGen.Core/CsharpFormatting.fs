module PocoGen.Core.CsharpFormatting
...
let buildClass (className : string) (dataRows : DataRow []) : string =
    ...
    let classDeclaration className = "public class " + className + "\r\n" + "{"
    ...
    let convertToPropertyString propValues =
        let typeValue =
            if propValues.isNullable then propValues.typeName + "?"
            else propValues.typeName
        sprintf "%s %s %s  {get;set;}" propValues.accessor typeValue
            propValues.columnName
